using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public static class CONSTANT 
{
    public readonly static string groundTag = "Ground";
    public readonly static string horizontalInput = "Horizontal";
    public readonly static string enemyTag = "Enemy";
    public readonly static string hitEvent = "HitEnd";
    public readonly static string playerPhysicsLayer = "Player";
    public readonly static string prefSave = "GameSaved";
    public readonly static string volumeButton = "VolumeButton";
    public readonly static string volumeSlider = "VolumeSlider";
    public readonly static string playButton = "PlayButton";
    public readonly static string finishLevelText = "FinishLevelText";
    public readonly static string bossChase = "BossChase";
    public readonly static string bossBlink = "BossBlink";
    public readonly static string bossGetHit = "BossGetHit";
     /* control player jump double jump wall jump slide, can pick player
      * enemy behaviour
      * map design: enemy, bonus, boss, stage pass, trap, player can shoot, UI, pick level, JSON firebase */
}
